...
namespace Fontendo.Extensions
{
    ...
    public class UnicodeNames
    {
        ...
        public UnicodeNames(string derivedNamePath = "")
        {
            if (derivedNamePath == "")
                derivedNamePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "DerivedName.txt");
            if (File.Exists(derivedNamePath))
            {
                try
                {
                    // Initialize the class by parsing DerivedName.txt
                    using var reader = new StreamReader(derivedNamePath);
